...
Por muitos anos, os modelos lineares convencionais foram amplamente empregados para tentar descrever uma variedade de fenômenos aleatórios.
Mesmo quando os dados em estudo não exibiam uma distribuição normal apropriada para as análises, frequentemente sugeria-se a aplicação de alguma forma de transformação para alcançar a normalidade desejada.

Essencialmente, a abordagem tradicional visava ampliar as opções de distribuição da variável de interesse.
Isso permitiria que a variável fosse incorporada à família de distribuições exponenciais uniparamétricas, possibilitando uma maior flexibilidade na relação funcional entre a média da variável e o preditor linear $\eta$.

Contudo, essa abordagem tinha suas limitações.
A busca incessante por normalidade nem sempre era viável, e a forçada adaptação dos dados a uma distribuição normal não se mostrava estatisticamente justificável.

À medida que o entendimento estatístico evoluiu, surgiu uma ampliação conceitual dos modelos lineares.
Surgiu, por exemplo, a classe dos Modelos Lineares Generalizados (MLGs), que englobam os modelos lineares tradicionais como casos particulares.
Os MLGs permitem acomodar diversas distribuições de respostas, não se restringindo apenas à normal.
Essa mudança de paradigma abriu espaço para a modelagem de dados que não seguem distribuições normais, como dados de contagem (Poisson), binomiais e proporções, entre outros.

A menção à "família exponencial uniparamétrica de distribuições" refere-se a um conjunto de distribuições que engloba várias distribuições comuns, incluindo a normal, Poisson, Binomial e Gama.
Essa família de distribuições possui propriedades matemáticas vantajosas, conferindo eficiência e flexibilidade à modelagem estatística.
...
Uma vantagem concreta é observada na garantia da unicidade da estimativa de máxima verossimilhança de ($\beta$), desde que essa estimativa exista.
...
**ca** - Quantidade de células com anomalias, dentre as células amostradas;  
...
Verifica-se que tanto a quantidade total de células quanto as que apresentaram anomalias têm uma distribuição assimétrica, porém a variabilidade das células anômalas é bastante menor. 
